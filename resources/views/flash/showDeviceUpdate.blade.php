...
@section('content')
    <div class="flash-cont flash-cont-show">
        <div class="flash-by-cont flash-by-cont-show">
            <ul id="{{ $device->id }}">
                <li>
                    {{ $device->price }} هزار تومان
                </li>
                <br />
            </ul>
            <form action="{{ route('addFileUpdShop', ['device'=>$device->id]) }}" method="GET">
                @csrf
                <button class="addFileUpdShop">افزودن به سبد خرید</button>
            </form>
        </div>
